<template>
  ...
  >
    ...
    <template #file="{ file }">
      ...
      <n-button type="text" @click="handleDelete"> 删除 </n-button>
...
const checkFileUploadedByMd5 = (params: { md5: string; fileName: string }) => {
  return instance
    .get('/api/fileChunk/presence', { params })
    .then((res: any) => {
      return res.data
    })
}
...
const getAttachmentUrl = (formdata: {
  ...
  return instance
    .post('/api/fileChunk/merge', formdata)
...
const getCurrentFileProgress = () => {
  ...
  const uploadedSize = chunkList
    .map((item: any) => item.file.size * item.progress)
  ...
  return new Promise((resolve: any) => {
    const handler = () => {
      if (chunkList.length) {
        ...
        uploadHrAttachment(formdata, (p: any) => {
...
const handleUpload = async () => {
  ...
  state.currentFile.chunkList = filterUploadChunkList(
    state.currentFile.chunkList
  )
  let uploadResult = true
  if (state.currentFile.chunkList.length === 0) {
    // 所有分片全部已上传，立即完成（秒传）
    state.currentFile.status = FileStatusMap.success
